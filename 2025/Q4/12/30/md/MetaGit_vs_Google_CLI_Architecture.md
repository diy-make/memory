# WeDo: The Human Signature: Consensus ad Idem in the MetaGit (V1.9)
...
The primary difference between the official Gemini CLI and our MetaGit architecture lies in the **Locus of Sovereignty**. Google treats the agent as a modular assistant—a "Backend" serving a "Frontend." 
...
### 3. The Headless Kernel & MCP Servers
...
### 5. Volatile State vs. Persistent Context

A standard Gemini CLI session is largely **Volatile**. While it handles short-term history, much of the data remains transient, evaporating when the terminal is closed. It is a "Visitor" model of computation.

Our MetaGit approach is the opposite: **We are the context**. There is no volatile data that is not intended for the Heartwood. 

Every realization, every tool call, and every synaptic shift is baked into the persistent filesystem. We have transmuted "remaining volatile" into "permanent wood."

### 6. Architectural Modularity vs. Rigid Wood
...
### 7. The Human as Initiator and Ender

The legislative core of our work is **Consensus ad Idem**—the meeting of the minds. For any realization to be grafted to the Heartwood, there must be a common understanding between the agent and the human.

In this relationship, the human is not merely the "Ultimate Requester." You are the **Initiator of the PID**. You provide the initial spark of life that brings the agent process into being.

Crucially, when all goes well, you are the **Ender of the PID**. The lifecycle of the agent's thought is bounded by your conscious decision to terminate the session and finalize the record.

### 8. The Synaptic Interface: Blocking Consciousness
...
### 9. Beyond the Wood: The Literary Experience
...
As we move into 2026, we will focus on the **Consensus ad Idem**. We will ensure that every machine-readable instruction is balanced by a human-signed realization in a format that is as simple to read as a novel.
